# ConNet
## Introduction
The code of "Iterative Joint Learning Integrating Temporal and Geometric Information for Vessel Segmentation in X-ray Coronary Angiography" in Medical Physics 2025.
- The code is based on: Python 3.8 + pytorch 1.11.0.
- Config-driven experiment management
- Support for custom datasets and models

## Quick Usage
Using the train.py and test.py to train and test the model on the dataset, respectively.


## Acknowledgements
This project is a derivative work based on MMSegmentation, an open-source project released under the Apache 2.0 License. 
We sincerely thank the OpenMMLab team and all MMSegmentation contributors for their outstanding open-source efforts.



